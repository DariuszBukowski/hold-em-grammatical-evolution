...
class game:
	...
	for i in range(8, -1, -1):
	    for j in range(0, 4):
		if reduce(operator.and_, map(lambda x: x in playersCards, range(i + j * 13, i + j * 13 + 5))):
		    return (8, i, None) # Straight flush
	suits = map(lambda x: x % 13, playersCards)
	...
	colors = map(lambda x: x / 13, playersCards)
	for i in range(0, 4):
	    if colors.count(i) >= 5:
		return (5, None, None) # Flush
	for i in range(8, -1, -1):
	    if reduce(operator.and_, map(lambda x: x in suits, range(i, i + 5))):
